...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	int return_code = alt_avalon_spi_command(
        SPI_0_BASE | CACHE_BYPASS, 0, // SPI_0 base address, slave address
...
void MAX3421E_reset(void) {
	...
	IOWR_ALTERA_AVALON_PIO_DATA(USB_RST_BASE | CACHE_BYPASS, 0);
	usleep(1000000);
	IOWR_ALTERA_AVALON_PIO_DATA(USB_RST_BASE | CACHE_BYPASS, 1);
	BYTE tmp = 0;
...
void MAX3421E_Task(void) {
	if ( IORD_ALTERA_AVALON_PIO_DATA(USB_IRQ_BASE | CACHE_BYPASS) == 0) {
		printf("MAX interrupt\n\r");
		MaxIntHandler();
	}
	if ( IORD_ALTERA_AVALON_PIO_DATA(USB_GPX_BASE | CACHE_BYPASS) == 1) {
